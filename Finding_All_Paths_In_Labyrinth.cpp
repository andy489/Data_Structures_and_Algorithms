...
void eraseLabyrinth()
{   // функция за освобождаване на динамично заделената памет за лабиринта
	for (int i = 0; i < rows; i++) delete[] labyrinth[i];
	delete[] labyrinth;
}
bool outOfLabyrinth(int row, int col)
{   // област на допустимите стойности
	return (row<0 || col<0 || row>rows - 1 || col>cols - 1) ? true : false;
}
bool isExit(int row, int col)
{   // стигнали сме до желаната дестинация
	return labyrinth[row][col] == 'e';
}
bool isPassable(int row, int col)
{   // ако е посетено или е стена върни лъжа, в противен случай върни истина
	return (labyrinth[row][col] == 'X' || labyrinth[row][col] == '*') ? false : true;
}
void printSolution()
{   // принтираме но без първия символ, който е стартовия S
