...
```
* Показания до тук ефикасен алгоритъм за сортиране използва за централна точка (pivot) последния (най-десния) елемент на колекцията от данни. За да напишем алгоритъм, който използва първия (най-лявия) елемент от колекцията може да се ***изхитрим*** и знаеики алгоритъма за разделяне на колекцията на две части спрямо централна точка последния елемент, просто преди да го приложим да слапнем последния елемент с първия. Аналогично ще е ако искаме централната точка да е средната (което е най-оптимално) или дори произволна (рандом) точка всеки път.
   * - за централна точка ср
``cpp
