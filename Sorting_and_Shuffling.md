...
```
* Показания до тук ефикасен алгоритъм за сортиране използва за централна точка (*pivot*) последния (най-десния) елемент на колекцията от данни. За да напишем алгоритъм, който използва първия (най-ляв) елемент от колекцията може да се ***изхитрим*** и знаеики алгоритъма за разделяне на колекцията на две части спрямо централна точка последния елемент, просто преди да го приложим да разменим (*swap-нем*) последния елемент с първия. Аналогично ще е ако искаме централната точка да е средната (което е най-оптимално) или дори произволна (*random*) точка всеки път.

** за ***деление с централна точка средния елемент*** може да приложим следното в началото на алгоритъма за разделяне *partition()*:  
```cpp
size_t midIndex= start + (end - start) / 2; // намираме позицията на средния елемент
	std::swap(arr[midIndex], arr[end]); // разменяме средния с последния елемент
``` 
** за ***деление с централна точка първия елемент***
