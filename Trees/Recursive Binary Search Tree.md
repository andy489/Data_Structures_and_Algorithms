...
В кода по-горе считаме, че в дървото не са позволени дубликати, защото използваме строго неравенство.

*!Съществува и трети подход за проверка на това дали дадено двоично дърво е двоично дърво за търсене или не. Може да приложим inorder traversal алгоритъм, т.е. да обходим дървото по метода DLR (Data-Left-Right depth-search algorithm) и ако дървото е бинарно дърво за търсене, то ще прочетем/принтираме елементите сортирани във възходящ ред. INorder Обхождането в бинарно дърво за търсене дава сортирана колекция. Може да приложим една проверка дали приемаме елементите в сортиран вид или не докато обхождаме. Необходимо е само да запазваме ключа по който сравняваме върховете на предходния връх и ако той е по-голям от следващия значи ще имаме индикатор, че дървото не е бинарно дърво за търсене.*

Виж още [BT & BST full](https://github.com/andy489/Data_Structures_and_Algorithms_CPP/blob/master/BT%20%26%20BST%20full.cpp).
