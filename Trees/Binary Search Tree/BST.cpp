...
BstNode* insert(BstNode* root, int data){
	if (root == nullptr){ //empty tree   
		root = getNewNode(data); //case: 1
	}
	else if (data <= root->data){ //case: 2
...
BstNode* minValueAddressNode(BstNode* root) { // recursive
	if (root == nullptr||root->left == nullptr) return root;
	return minValueAddressNode(root->left);
}

BstNode* minValAddNode(BstNode* root) { // iterative
...
BstNode* maxValueAddressNode(BstNode* root) { // recursive
