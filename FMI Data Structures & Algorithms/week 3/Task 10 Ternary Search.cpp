/* Our function that calculates the minimum usage of monsters
for moving the trucks in one line is QUADRATIC, so if we assume
...
ll getCost(const vector<Truck>& trucks, int pos)
{    //for a position "pos" our answer will be "ans"
	int N = sz(trucks);
	ll ans(0);
	F(i, 0, N)
	{
...
ll search(const vector<Truck>& trucks, int l, int r)
{    // left = left most bound; right = right most bound
	if (r - l < 3)
	{
...
int main()
{
	...
	if (N > 1)
	{
		cout << search(trucks, minBound, maxBound) << '\n';
	}
